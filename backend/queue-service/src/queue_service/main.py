"""Application entry point for the queue-service.

Wires up FastAPI, logging, database, the queue service, and optional Kafka
producer for queue events (local development runs without a bus).
"""
...
from queue_service.api.routes import router
from queue_service.configuration import get_settings
from queue_service.entity.models import Base
from queue_service.service.queue_service import QueueError, QueueService
...
async def lifespan(app: FastAPI):
    ...
    except Exception:  # noqa: BLE001 - no bus in local dev; queuing still works
        app.state.producer_error = True
        producer = None

    app.state.database = database
    app.state.settings = settings
    app.state.queue_service = QueueService(settings, producer=producer)
...
def create_app() -> FastAPI:
    ...
    async def handle_queue_error(_request: Request, exc: QueueError) -> JSONResponse:
        return JSONResponse(
