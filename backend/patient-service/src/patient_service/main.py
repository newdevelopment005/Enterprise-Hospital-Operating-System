"""Application entry point for the patient-service.

Wires up FastAPI, logging, database, the patient service, and optional Kafka
producer for patient events (local development runs without a bus).
"""
...
from patient_service.api.routes import router
from patient_service.configuration import get_settings
...
async def lifespan(app: FastAPI):
    ...
    except Exception:  # noqa: BLE001 - no bus in local dev; registration still works
        app.state.producer_error = True
        producer = None

    app.state.database = database
    app.state.settings = settings
    app.state.patient_service = PatientService(settings, producer=producer)
...
def create_app() -> FastAPI:
    ...
    @app.exception_handler(PatientError)
    async def handle_patient_error(_request: Request, exc: PatientError) -> JSONResponse:
        return JSONResponse(
