"""Application entry point for the EHOS prediction-service."""
...
from prediction_service.api.routes import router
from prediction_service.configuration import get_settings
from prediction_service.entity.models import Base
from prediction_service.service.provider import PredictionError, PredictionService
...
async def lifespan(app: FastAPI):
    ...
    except Exception:  # noqa: BLE001 - no bus in local dev; forecasts still served
        app.state.producer_error = True
        producer = None

    app.state.database = database
    app.state.settings = settings
    app.state.prediction_service = PredictionService(settings)
...
def create_app() -> FastAPI:
    ...
    async def handle_prediction_error(_request: Request, exc: PredictionError) -> JSONResponse:
